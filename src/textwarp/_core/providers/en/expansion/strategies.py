...
def _expand_inverted_negative(
    ...
    """
    Handle expansion for inverted negative contractions (e.g., "Didn't I").

    Args:
        doc: The spaCy `Doc` containing the tokens.
        span: The spaCy `Span` containing the contraction.
        subject_token: The spaCy `Token` for the subject of the
            contraction.
        base_verb: The base form of the verb to use in the expansion.
    ...
    return f'{base_verb} {intermediate_text}not', return_idx
...
def expand_negative_contraction(span: Span) -> tuple[str, int] | None:
    ...
    subject_token = en.utils.find_subject_token(prev_token)

    if not subject_token and span.end < len(doc):
        next_token = doc[span.end]
        if next_token.pos_ in {'PROPN', 'PRON'} or next_token.is_title:
            subject_token = next_token

    if subject_token and subject_token.i > prev_token.i:
        expanded_text, return_idx = _expand_inverted_negative(
            doc, span, subject_token, base_verb
        )
    else:
