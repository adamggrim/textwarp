...
def get_common_stateless_participles() -> tuple[str, ...]:
    ...
    return tuple(
        load_json_data(DIR / 'common_stateless_participles.json', locale='en')
    )
...
def get_unambiguous_map() -> Mapping[str, str]:
    ...
    return MappingProxyType(
        load_json_data(DIR / 'unambiguous_contractions_map.json', locale='en')
    )
...
def get_whatcha_are_words() -> tuple[str, ...]:
    ...
    """
    return tuple(load_json_data(DIR / 'whatcha_are_words.json', locale='en'))
...
def get_whatcha_have_words() -> tuple[str, ...]:
    ...
    """
    return tuple(load_json_data(DIR / 'whatcha_have_words.json', locale='en'))
