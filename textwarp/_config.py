...
def _load_json_from_data(filename: str) -> JSONType:
    """
...
CAPITALIZED_ABBREVIATIONS_MAP: Final[dict[str, str]] = cast(
    dict[str, str], _load_json_from_data('capitalized_abbreviations_map.json')
)

# Suffix tokens derived from contractions.
CONTRACTION_SUFFIX_TOKENS: Final[list[str]] = cast(
    list[str], _load_json_from_data('contraction_suffix_tokens.json')
)

# Map pairing each contraction with its expanded version.
CONTRACTIONS_MAP: Final[dict[str, str]] = cast(
    dict[str, str], _load_json_from_data('contractions_map.json')
)
...
OTHER_PREFIXED_NAMES_MAP: Final[dict[str, str]] = cast(
    dict[str, str], _load_json_from_data('other_prefixed_names_map.json')
)
